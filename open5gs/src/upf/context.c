...
static int upf_context_validation(void)
{
    if (ogs_list_first(&ogs_gtp_self()->gtpu_list) == NULL) {
        if (upf_self()->transport_mode == UPF_TRANSPORT_MODE_QUIC) {
            ogs_info("[QUIC N3] No GTPU address found, but QUIC is enabled. Proceeding...");
        } else {
    ...
    while (ogs_yaml_iter_next(&root_iter)) {
        ...
        if ((!strcmp(root_key, "upf")) &&
            ...
            while (ogs_yaml_iter_next(&upf_iter)) {
                ...
                } else if (!strcmp(upf_key, "transport_mode"))
                {
                    const char *value = ogs_yaml_iter_value(&upf_iter);
                    if (value && !strcmp(value, "quic"))
                    {
                        upf_self()->transport_mode = UPF_TRANSPORT_MODE_QUIC;
                        ogs_info("UPF Transport Mode set to: QUIC");
                    }else
                    {
                        upf_self()->transport_mode = UPF_TRANSPORT_MODE_GTPU;
                        ogs_info("UPF Transport Mode set to: Standard GTP-U");
                    }
                }else if (!strcmp(upf_key, "quic"))
                {
                    ogs_yaml_iter_t q_iter;
                    ogs_yaml_iter_recurse(&upf_iter, &q_iter);

                    while (ogs_yaml_iter_next(&q_iter))
                    {
                        const char *q_key = ogs_yaml_iter_key(&q_iter);
                        const char *q_value = ogs_yaml_iter_value(&q_iter);

                        if (!strcmp(q_key, "cert"))
                        {
                            upf_self()->quic_cert_path = ogs_strdup(q_value);
