...
class SendContextPool
{
  ...
  public:
    ...
    {
        assert(size == m_blockSize && "SendContextPool only supports single object allocation");
        if (size != m_blockSize)
...
SendContextPool::SendContextPool() : m_capacity(4096), m_blockSize(sizeof(SendContext))
{
...
void QuicTask::onStart()
{
    ...
    if (QUIC_FAILED(status = m_msQuicApi->ConfigurationOpen(m_registration, &alpnBuffer, 1, &settings, sizeof(settings),
...
void QuicTask::connect(const InetAddress &to)
{
    ...
    if (m_savedResumptionTicketLength > 0)
    {
        if (QUIC_FAILED(status = m_msQuicApi->SetParam(m_connection, QUIC_PARAM_CONN_RESUMPTION_TICKET,
                                                       m_savedResumptionTicketLength, m_savedResumptionTicket)))
...
void QuicTask::send(NmGnbGtpToQuic *w)
{
    ...
    if (m_quicState == QuicState::DISCONNECTED)
    {
        m_quicState = QuicState::CONNECTING;
        m_pendingContexts.push(ctx);
        m_logger->debug("State: DISCONNECTED -> CONNECTING. Queueing first packet.");
        connect(w->ip);
    }
    else if (m_quicState == QuicState::CONNECTING)
    {
        // Connection is still shaking hands. Queue it up.
        m_pendingContexts.push(ctx);
    }
    else if (m_quicState == QuicState::CONNECTED)
    {
        executeSend(ctx);
    }
}

void QuicTask::executeSend(SendContext *ctx)
{
    if (QUIC_FAILED(status = m_msQuicApi->DatagramSend(m_connection, &ctx->quicBuffer, 1,
                                                       QUIC_SEND_FLAG_CANCEL_ON_BLOCKED, ctx)))
    {
...
QUIC_STATUS QUIC_API QuicTask::connectionCallback(HQUIC conn, void *context, QUIC_CONNECTION_EVENT *event)
{
    ...
    case QUIC_CONNECTION_EVENT_CONNECTED: {
        ...
        std::lock_guard<std::mutex> lock(ctx->m_stateMutex);
        ctx->m_quicState = QuicState::CONNECTED;

        ctx->m_logger->debug("Flushing %zu queued packets.", ctx->m_pendingContexts.size());
        while (!ctx->m_pendingContexts.empty())
        {
            auto *sendCtx = ctx->m_pendingContexts.front();
            ctx->m_pendingContexts.pop();
            ctx->executeSend(sendCtx);
        }
        break;
    }
    ...
    case QUIC_CONNECTION_EVENT_SHUTDOWN_COMPLETE: {
        ...
        std::lock_guard<std::mutex> lock(ctx->m_stateMutex);
        ctx->m_quicState = QuicState::DISCONNECTED;

        while (!ctx->m_pendingContexts.empty())
        {
            delete ctx->m_pendingContexts.front();
            ctx->m_pendingContexts.pop();
        }
        break;
    }
