...
GNodeB::GNodeB(GnbConfig *config, app::INodeListener *nodeListener, NtsTask *cliCallbackTask)
{
    ...
    base->quicTask = config->transportMode == QUIC ? new QuicTask(base) : nullptr;
...
GNodeB::~GNodeB()
{
    ...
    if (taskBase->quicTask)
        taskBase->quicTask->quit();
